...
The co-cddo fork of Innovation Sandbox on AWS is currently at version v1.1.4 (SHA `cf75b87`) and is **10 commits behind** the upstream `aws-solutions/innovation-sandbox-on-aws` (SHA `90488e0`). The fork contains no source code modifications -- CDDO uses an extension architecture with external satellite services rather than modifying the upstream codebase. The 10 missing upstream commits represent versions v1.1.5 through v1.1.7, consisting entirely of security patches and a dependency upgrade.
...
| **Upstream SHA** | `90488e0` |
| **Fork SHA** | `cf75b87` |
| **Commits Ahead** | 0 |
| **Commits Behind** | 10 |
| **Checked At** | 2026-03-02T23:25:00Z |
| **Fork Version** | v1.1.4 (2025-12-16) |
| **Upstream Version** | v1.1.7 (2026-01-20) |
...
gitGraph
    ...
    commit id: "v1.1.7" tag: "v1.1.7 (upstream HEAD)"
```

---

## Missing Upstream Changes (v1.1.5 -- v1.1.7)

The 10 commits behind upstream represent three patch releases. All are security-only releases with no feature changes.
...
The fork should be updated to v1.1.7 to receive:

1. **aws-nuke v3.63.2** (v1.1.7) -- fixes SCP-protected log group deletion failures during account cleanup. This directly impacts the NDX pool account recycling workflow and may be the cause of some accounts being stuck in the Quarantine OU.

2. **Security patches** (v1.1.5, v1.1.6) -- qs library vulnerability, @remix-run/router, glib2, libcap, python3 upgrades.
...
*Generated from source analysis on 2026-03-02. See [00-repo-inventory.md](./00-repo-inventory.md) for full repository inventory.*
